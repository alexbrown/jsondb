...
test('should return an array of all the records in a collection', async () => {
    ...
    expect(res.message[0].collection).toBe('users');
    expect(res.message[0].content).toHaveProperty('name');
    expect(res.message[0].content.name).toBe('Larry');
